...
import com.jinkeloid.yellowbananapixeldungeon.visuals.ui.CheckBox;
...
public class WndSettings extends Window {
	...
	public WndSettings( final boolean inGame ) {
		...
		if (inGame) {
            ...
            RedButton btnSearchBtn = new RedButton( searchButtonsText( YellowBananaPixelDungeon.searchButton() ) ) {
                @Override
                protected void onClick(){

                    boolean val = !YellowBananaPixelDungeon.searchButton();

                    YellowBananaPixelDungeon.searchButton( val );
            ...
            CheckBox btnBrightness = new CheckBox( TXT_BRIGHTNESS ) {
                ...
                protected void onClick() {
                    super.onClick();
                    YellowBananaPixelDungeon.brightness(checked());
                }
            };
            btnBrightness.setRect(0, btnSearchBtn.bottom()+ GAP, WIDTH, BTN_HEIGHT);
            btnBrightness.checked(YellowBananaPixelDungeon.brightness());
            add(btnBrightness);
            ...
            RedButton btnOrientation = new RedButton( orientationText() ) {
                @Override
                protected void onClick() {
                    YellowBananaPixelDungeon.landscape(!YellowBananaPixelDungeon.landscape());
                }
            };
            ...
            CheckBox btnScaleUp = new CheckBox( TXT_SCALE_UP ) {
                ...
                protected void onClick() {
                    super.onClick();
                    YellowBananaPixelDungeon.scaleUp(checked());
                }
            };

            btnScaleUp.setRect(0, btnOrientation.bottom() + GAP, WIDTH, BTN_HEIGHT);
            btnScaleUp.checked(YellowBananaPixelDungeon.scaleUp());
            add( btnScaleUp );
            ...
            btnImmersive = new CheckBox( TXT_IMMERSIVE ) {
                ...
                protected void onClick() {
                    super.onClick();
                    YellowBananaPixelDungeon.immerse(checked());
                }
            };
            btnImmersive.setRect(0, btnScaleUp.bottom() + GAP, WIDTH, BTN_HEIGHT);
            btnImmersive.checked(YellowBananaPixelDungeon.immersed());
		...
		CheckBox btnMusic = new CheckBox( TXT_MUSIC ) {
			...
			protected void onClick() {
				super.onClick();
				YellowBananaPixelDungeon.music(checked());
			}
		};
		btnMusic.setRect( 0, ( BTN_HEIGHT + GAP ) * 3, WIDTH, BTN_HEIGHT );
		btnMusic.checked( YellowBananaPixelDungeon.music() );
		add(btnMusic);
        ...
        CheckBox btnSound = new CheckBox( TXT_SOUND ) {
            ...
            protected void onClick() {
                super.onClick();
                YellowBananaPixelDungeon.soundFx(checked());
                Sample.INSTANCE.play(Assets.SND_CLICK);
            }
        };
        btnSound.setRect(0, btnMusic.bottom() + GAP, WIDTH, BTN_HEIGHT);
        btnSound.checked(YellowBananaPixelDungeon.soundFx());
        add(btnSound);
...
//        add(btnTracks);
        ...
        RedButton btnTipsDelay = new RedButton( loadingTipsText( YellowBananaPixelDungeon.loadingTips() ) ) {
            @Override
            protected void onClick() {

                int val = YellowBananaPixelDungeon.loadingTips();

                val = val < 3 ? val + 1 : 0;
                YellowBananaPixelDungeon.loadingTips(val);
...
//			resize( WIDTH, (int)btnQuickslot.bottom() );
	...
	private void zoom( float value ) {
		...
		YellowBananaPixelDungeon.zoom((int) (value - PixelScene.defaultZoom));
	...
	private String orientationText() {
		return YellowBananaPixelDungeon.landscape() ? TXT_SWITCH_PORT : TXT_SWITCH_LAND;
	}
